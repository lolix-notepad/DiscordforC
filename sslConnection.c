...
int main(int count, char *strings[]) {
  ...
  ShowCerts(ssl,outbio);
    ...
    // char *start_line             = "POST /api/channels/898466977645994005/messages HTTP/1.1";
    char *start_line             = "POST /api/channels/898466977645994004/messages HTTP/1.1";
    ...
    char *header_authorization   = "Authorization: Bot ODk4NDQxNTkwODg3MDQ3MTY4.YWkQ7w.jx3s9C8SS_iLPdK9lvydb27hgEI";
    char *header_content_length  = "Content-Length: ";
    char *json                   = "{\"content\":\"Hello World, from C\"}";
    char *end_line               = "\r\n";


    strcpy(mesg, start_line);
    // strcat(mesg, start_line); this is wrong because you are allowing the memory
    // but I am not pretty why but there appears random characters, so you must clean it like
    // type *ptr = malloc(1024)

    // ptr = "hello"
    // strcpy(ptr, "hello")
    strcat(mesg, end_line);

    strcat(mesg, header_host);
    strcat(mesg, end_line);

    /* strcat(mesg, header_accept);
    strcat(mesg, end_line); */

    strcat(mesg, header_content_type);
    strcat(mesg, end_line);

    strcat(mesg, header_authorization);
    strcat(mesg, end_line);

    strcat(mesg, header_content_length);
    sprintf(mesg, "%s%d", mesg, strlen(json));
    // strcat(mesg, (char*) strlen(json));
    strcat(mesg, end_line);
    strcat(mesg, end_line);
    ...
    // strcat(mesg, "GET /api/gateway HTTP/1.1\r\n\r\n");
